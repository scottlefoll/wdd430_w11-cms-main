...
export class ContactService{
  ...
  addContact(contact: Contact){
    ...
    // add to database
    this.http.post<{message: string, contact: Contact}>(`${environment.api2Url}/contacts`, contact, {headers: headers})
      .subscribe(
        (responseData) => {
  ...
  getContacts(forceReload: boolean = false) {
    if (!this.contactsLoaded || forceReload) {
      this.http.get<{contacts: Contact[]}>(`${environment.api2Url}/contacts`)
        .subscribe(
          (response) => {
  ...
  updateContact(originalContact: Contact, updatedContact: Contact){
    ...
    // Update the database
    this.http.put(`${environment.api2Url}/contacts/${originalContact.id}`, updatedContact, {headers: headers})
      .subscribe(
        (response) => {
          // this.getContacts(); // Fetch the updated contacts list from the server
  ...
  deleteContact(contact: Contact) {
    ...
    this.http.delete(`${environment.api2Url}/contacts/${contact.id}`)
      .subscribe(
        (response) => {
          // this.getContacts(); // Fetch the updated contacts list from the server
